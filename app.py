...
def plan_vocnjak(kultura, mesec):
    plan = {
        "Voće": {
            "Maj": [
                ("Captan", "35 g / 16 L", "Preventiva protiv gljivičnih bolesti"),
                ("Bor", "folijarno po etiketi", "Poboljšanje oplodnje")
            ],
            "Jun": [
                ("Coragen", "3 ml / 16 L", "Suzbijanje smotavca"),
                ("Wuxal Calcium", "40 ml / 16 L", "Prihrana kalcijumom")
            ],
            "Jul": [
                ("Kalcijum", "po etiketi", "Kvalitet ploda i čvrstina"),
                ("Biostimulator", "po etiketi", "Ublažavanje stresa")
            ],
            "Avgust": [
                ("Teldor", "15 ml / 16 L", "Zaštita od truleži pred berbu")
            ],
            "Septembar": [
                ("Bakreni preparat", "po etiketi", "Higijenska zaštita posle berbe")
            ],
            "Oktobar": [
                ("NPK 6:12:24", "po etiketi", "Jesenje đubrenje")
            ]
        }
    }
...
def plan_povrce(kultura, mesec):
    plan = {
        "Paradajz": {
            "Maj": [
                ("Bakarni preparat", "po etiketi", "Prevencija plamenjače"),
                ("Kalcijum", "30-40 ml / 16 L", "Jačanje ploda i lista")
            ],
            "Jun": [
                ("Bakarni preparat", "po etiketi", "Zaštita lista"),
                ("Sumporni preparat", "po etiketi", "Ako je suvo vreme")
            ],
            "Jul": [
                ("Kalcijum", "30-40 ml / 16 L", "Kvalitet ploda"),
                ("Biostimulator", "po etiketi", "Ublažavanje stresa")
            ]
        },
        "Paprika": {
            "Maj": [
                ("Kalcijum", "30-40 ml / 16 L", "Prevencija ožegotina"),
                ("Biostimulator", "po etiketi", "Oporavak od presadnje")
            ],
            "Jun": [
                ("Kalcijum", "30-40 ml / 16 L", "Kvalitet ploda"),
                ("Blagi fungicid", "po etiketi", "Prevencija bolesti lista")
            ]
        },
        "Krastavac": {
            "Maj": [
                ("Sumporni preparat", "po etiketi", "Pepelnica"),
                ("Biološki preparat", "po etiketi", "Lakša zaštita")
            ],
            "Jun": [
                ("Sumporni preparat", "po etiketi", "Pepelnica"),
                ("Kalcijum", "30 ml / 16 L", "Jačanje ploda")
            ]
        },
        "Krompir": {
            "Maj": [
                ("Mankozeb", "po etiketi", "Preventiva protiv plamenjače"),
                ("Bakreni preparat", "po etiketi", "Dodatna zaštita")
            ],
            "Jun": [
                ("Mankozeb", "po etiketi", "Zaštita lista"),
                ("Insekticid za zlaticu", "po etiketi", "Suzbijanje štetočine")
            ]
        },
        "Luk": {
            "Maj": [
                ("Bakreni preparat", "po etiketi", "Zaštita od bolesti lista"),
                ("Kalcijum", "po etiketi", "Jačanje biljnog tkiva")
            ],
            "Jun": [
                ("Bakreni preparat", "po etiketi", "Prevencija bolesti"),
                ("Fungicid", "po etiketi", "Ako je vlažno vreme")
            ]
        },
        "Lubenica": {
            "Maj": [
                ("Kalcijum", "30 ml / 16 L", "Kvalitet i otpornost"),
                ("Biostimulator", "po etiketi", "Ukorenjavanje")
            ],
            "Jun": [
                ("Kalcijum", "30 ml / 16 L", "Kvalitet ploda"),
                ("Preventivni fungicid", "po etiketi", "Zaštita lista")
            ]
        },
        "Boranija": {
            "Maj": [
                ("Bakreni preparat", "po etiketi", "Preventiva bolesti"),
                ("Biostimulator", "po etiketi", "Bolji početni rast")
            ],
            "Jun": [
                ("Blagi fungicid", "po etiketi", "Zaštita lista"),
                ("Kalcijum", "po etiketi", "Jačanje ploda")
            ]
        },
        "Grašak": {
            "Maj": [
                ("Blagi fungicid", "po etiketi", "Zaštita cveta i lista"),
                ("Kalcijum", "po etiketi", "Jačanje biljke")
            ],
            "Jun": [
                ("Fungicid", "po etiketi", "Ako je vlažno vreme"),
                ("Biostimulator", "po etiketi", "Oporavak od stresa")
            ]
        }
    }
...
with tab1:
    ...
    kultura_v = st.selectbox(
        "Kultura u voćnjaku:",
        ["Voće"],
        key="kultura_v"
    )

    tretmani_v = plan_vocnjak(kultura_v, v_mesec)
    st.subheader("Preporučeni tretmani")
    if tretmani_v:
        for naziv, doza, namena in tretmani_v:
            st.info(f"**{naziv}** — {doza} — {namena}")
    else:
        st.warning("Za ovaj mesec nema unetih preporuka.")
...
with tab2:
    ...
    mesec_p = st.selectbox("Mesec:", ["Maj", "Jun", "Jul", "Avgust", "Septembar", "Oktobar"], key="mesec_p")
    baza_p = {
    ...
    tretmani_p = plan_povrce(kultura, mesec_p)
    st.subheader("Preporučeni tretmani")
    if tretmani_p:
        for naziv, doza, namena in tretmani_p:
            st.info(f"**{naziv}** — {doza} — {namena}")
    else:
        st.warning("Za ovu kulturu i mesec nema unetih preporuka.")
